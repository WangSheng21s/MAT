...
""" 该实验加了ent-marker,第一种实验方式表示ent-marker只能看到tokens，而且re-marker只能看到对应的ent-marker，第二种实验方式表示ent-marker只能看到tokens，但是re-marker能够看到所有的ent-marker,第三种方式表示ent-marker能看到所有的tokens和re-marker,但是re-marker只能看到对应的ent-marker,第四种方式类似于3，但是re-marker能够看到所有的entmarker,第五种方式re-marker不能看到ent-marker,用于类比对照"""
...
MODEL_CLASSES = {
    'bertsub': (BertConfig, run_re_shiyan2, BertTokenizer),
...
class ACEDataset(Dataset):
    def __init__(self, tokenizer, args=None, evaluate=False, do_test=False, max_pair_length=None):
        ...
        self.max_ent_marker_length = 2 * args.max_ent_length
        self.max_pair_length = max_pair_length
        #self.max_entity_length = self.max_pair_length*6#每对关系由6个marker组成
        self.max_entity_length = self.max_pair_length * 4  # 每对关系由6个marker组成
        ...
        self.shiyanfangshi = args.shiyanfangshi
            ...
            for n in range(len(subword_sentence_boundaries) - 1):
                ...
                for sub in entities:
                    ...
                    for start, end, obj_label in sentence_ners:
                        ...
                        cur_ins.append(((sub_s, sub_e, sub_label), (left, right, ner_label_map[obj_label]), label,
                                        (sub[0], sub[1]), obj))
                ...
                for i in range(0, len(cur_ins), dL):
                    ...
                    item = {
                        ...
                        'examples': examples,
                        #'start_end_entities': start_end_entities#start,end label
                    }
    ...
    def __getitem__(self, idx):
        ...
        #start_end_entities = entry['start_end_entities']
        #len_entities = min( len(start_end_entities), self.max_ent_marker_length // 2)
        #attention_mask = torch.zeros((self.max_entity_length+self.max_seq_length+self.max_ent_marker_length, self.max_entity_length+self.max_seq_length+self.max_ent_marker_length), dtype=torch.int64)
        ...
        #3,4给ent-marker，其余的给5,6,7,8给re-marker
        if self.model_type.startswith('albert'):#每一对关系需要六个marker表示，就用sub1,sub2,obj1,obj2,context1,context2表示吧
            #input_ids = input_ids + [30003] * (len_entities) + [self.tokenizer.pad_token_id] * (self.max_ent_marker_length//2 - len_entities)
            #input_ids = input_ids + [30002] * (len_entities) + [self.tokenizer.pad_token_id] * (self.max_ent_marker_length//2 - len_entities)
            input_ids = input_ids + [30004] * (len(entry['examples'])) + [self.tokenizer.pad_token_id] * (self.max_pair_length - len(entry['examples']))
            input_ids = input_ids + [30005] * (len(entry['examples'])) + [self.tokenizer.pad_token_id] * (self.max_pair_length - len(entry['examples']))
            input_ids = input_ids + [30006] * (len(entry['examples'])) + [self.tokenizer.pad_token_id] * (self.max_pair_length - len(entry['examples']))
            input_ids = input_ids + [30007] * (len(entry['examples'])) + [self.tokenizer.pad_token_id] * (self.max_pair_length - len(entry['examples']))
        else:
            #input_ids = input_ids + [3] * (len_entities) + [self.tokenizer.pad_token_id] * (self.max_ent_marker_length//2 - len_entities)
            #input_ids = input_ids + [4] * (len_entities) + [self.tokenizer.pad_token_id] * (self.max_ent_marker_length//2 - len_entities)
            ...
            input_ids = input_ids + [5] * (len(entry['examples'])) + [self.tokenizer.pad_token_id] * (self.max_pair_length - len(entry['examples']))
            input_ids = input_ids + [6] * (len(entry['examples'])) + [self.tokenizer.pad_token_id] * (self.max_pair_length - len(entry['examples']))


        rel_labels = []
        #start_end_entities_labels = []
        position_ids = list(range(self.max_seq_length)) + [0] * (self.max_entity_length )
        num_pair = self.max_pair_length
        sub_labels, obj_labels = [], []
        sub_mention_pos, obj_mention_pos = [], []
        sub_mention2, obj_mention2 = [], []

        #for i in range(len_entities):
        #    entity = start_end_entities[i]
        #    start_end_entities_labels.append(start_end_entities[i][2])
        #    w1 = self.max_seq_length + i
        #    w2 = w1 + self.max_ent_marker_length//2
        #    position_ids[w1] = start_end_entities[i][0]
        #    position_ids[w2] = start_end_entities[i][1]
        #    if self.shiyanfangshi == 1 or self.shiyanfangshi == 2:
        #        for xx in [w1, w2]:
        #            for yy in [w1, w2]:
        #                attention_mask[xx, yy] = 1
        #            attention_mask[xx, :L] = 1
        #    else:#shiyanfangshi==3  or 4
        #        for xx in [w1, w2]:
        #            for yy in [w1,w2]:
        #                attention_mask[xx,yy] = 1
        #            attention_mask[xx, :L] = 1#ent-marker可以看到tokens
        #            attention_mask[xx, self.max_seq_length+self.max_ent_marker_length:] = 1
        #    if self.use_typemarker:
        #        sub_l_m = '[unused%d]' % ( 2 + entity[2] + len(self.ner_label_list)*2 )
        #        sub_r_m = '[unused%d]' % ( 2 + entity[2] + len(self.ner_label_list)*3 )
        #        sub_l_m = self.tokenizer._convert_token_to_id(sub_l_m)
        #        sub_r_m = self.tokenizer._convert_token_to_id(sub_r_m)
        #        input_ids[w1] = sub_l_m
        #        input_ids[w2] = sub_r_m




        #cur_ins.append(( (sub_s,sub_e,sub_label),(left, right, ner_label_map[obj_label]), label, (sub[0], sub[1]), obj))
        for x_idx, sub_obj in enumerate(entry['examples']):
            #m2 = obj[0]
            #sub_idx, obj_idx = sub_obj[0], sub_obj[1]
            sub, obj = sub_obj[0], sub_obj[1]
            label = sub_obj[2]

            sub_mention_pos.append((sub[0], sub[1]))
            obj_mention_pos.append((obj[0], obj[1]))

            sub_mention2.append(sub_obj[3])
            obj_mention2.append(sub_obj[4])

            w1 = x_idx + self.max_seq_length
            w2 = w1 + num_pair  # 1,2表示sub,3,4表示obj，5,6表示context
            w3 = w2 + num_pair
            w4 = w3 + num_pair

            position_ids[w1] = sub[0]
            position_ids[w2] = sub[1]
            position_ids[w3] = obj[0]
            position_ids[w4] = obj[1]


            #if self.shiyanfangshi == 1 or self.shiyanfangshi ==3:
            #    for xx in [w1, w2, w3, w4]:
            #        for yy in [w1, w2, w3,w4]:
            #            attention_mask[xx, yy] == 1
            #        attention_mask[xx, :L] = 1
            #        attention_mask[xx, self.max_seq_length + 2 * sub_idx:self.max_seq_length + 2 * sub_idx + 2] = 1
            #        attention_mask[xx, self.max_seq_length + 2 * obj_idx:self.max_seq_length + 2 * obj_idx + 2] = 1#re-marker可以看到ent-marker
            #elif self.shiyanfangshi ==2 or self.shiyanfangshi == 4:
            #    for xx in [w1, w2, w3, w4]:
            #        for yy in [w1, w2, w3,w4]:
            #            attention_mask[xx, yy] == 1
            #        attention_mask[xx, :L] = 1
            #        attention_mask[xx, self.max_seq_length :self.max_seq_length + self.max_ent_marker_length] = 1
            #shiyanfangshi==5
            #print("shiyanfangshi 的值为5")
            for xx in [w1, w2, w3, w4]:
                for yy in [w1, w2, w3,w4]:
                    attention_mask[xx, yy] == 1
                attention_mask[xx, :L] = 1
            ...
            sub_labels.append(sub[2])
            obj_labels.append(obj[2])

            if self.use_typemarker:
                sub_l_m = '[unused%d]' % ( 2 + sub[2] + len(self.ner_label_list)*2 )
                sub_r_m = '[unused%d]' % ( 2 + sub[2] + len(self.ner_label_list)*3 )
                sub_l_m = self.tokenizer._convert_token_to_id(sub_l_m)
                sub_r_m = self.tokenizer._convert_token_to_id(sub_r_m)
                input_ids[w1] = sub_l_m
                input_ids[w2] = sub_r_m

                obj_l_m = '[unused%d]' % (2 + obj[2] + len(self.ner_label_list) * 2)
                obj_r_m = '[unused%d]' % (2 + obj[2] + len(self.ner_label_list) * 3)
                obj_l_m = self.tokenizer._convert_token_to_id(obj_l_m)
                obj_r_m = self.tokenizer._convert_token_to_id(obj_r_m)
                input_ids[w3] = obj_l_m
                input_ids[w4] = obj_r_m

        pair_L = len(entry['examples'])

        sub_mention_pos += [(0, 0)] * (num_pair - len(sub_mention_pos))
        obj_mention_pos += [(0, 0)] * (num_pair - len(obj_mention_pos))
        mention_pos = sub_mention_pos + obj_mention_pos

        rel_labels += [-1] * (num_pair - len(rel_labels))

        sub_labels += [-1] * (num_pair - len(sub_labels))
        obj_labels += [-1] * (num_pair - len(obj_labels))

        #start_end_entities_labels += [-1] * (self.max_ent_marker_length//2 - len(start_end_entities_labels))
        ner_labels = sub_labels + obj_labels

        item = [torch.tensor(input_ids),
                attention_mask,
                torch.tensor(position_ids),
                torch.tensor(mention_pos),
                torch.tensor(rel_labels, dtype=torch.int64),
                torch.tensor(ner_labels, dtype=torch.int64),
                #torch.tensor(start_end_entities_labels, dtype=torch.int64)
        ]

        if self.evaluate:
            #item.append(start_end_entities)
            item.append(entry['index'])
            mention2 = sub_mention2 + obj_mention2
            item.append(mention2)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {'input_ids':      batch[0],
                      ...
                      'labels':         batch[4],
                      #'start_end_entities_labels':batch[6],
                      'ner_labels':     batch[5],
                      }
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            for i in range(len(indexs)):
                ...
                for j in range(len(m2s) // 2):
                    sub = m2s[j]
                    obj = m2s[j + len(m2s)//2]

                    sub_label = eval_dataset.ner_label_list[ner_preds[i, j]]
                    obj_label = eval_dataset.ner_label_list[ner_preds[i, j+args.max_pair_length]]
                    scores[(index[0], index[1])][((sub[0], sub[1]), (obj[0], obj[1]))] = (
                    logits[i, j].tolist(), sub_label, obj_label)
    ...
    if not args.eval_unidirect:  # eval_unidrect is for ablation study
        ...
        for example_index, pair_dict in sorted(scores.items(), key=lambda x: x[0]):
            visited = set([])
            sentence_results = []
            for k1, (v1, v2_sub_label, v2_obj_label) in pair_dict.items():

                if k1 in visited:
                    continue
                visited.add(k1)

                if v2_sub_label == 'NIL' or v2_obj_label == 'NIL':
                    continue
                v1 = list(v1)
                m1 = k1[0]
                m2 = k1[1]
                ...
                if v2s is not None:
                    visited.add(k2)
                    v2, v1_sub_label, v1_obj_label = v2s
                ...
                if v1_obj_label == 'NIL' or v1_sub_label == 'NIL':
                    continue
                ...
                if pred_label > 0:
                    if pred_label >= num_label:
                        ...
                        m1, m2 = m2, m1
                        v1_obj_label, v2_obj_label = v2_obj_label, v1_obj_label

                    pred_score = v1[pred_label]

                    sentence_results.append((pred_score, m1, m2, pred_label, v1_obj_label, v2_obj_label))
        ...
        for example_index, pair_dict in sorted(scores.items(), key=lambda x: x[0]):
            ...
            for k1, (v1, v2_sub_label, v2_obj_label) in pair_dict.items():
                ...
                if pred_label > 0 and pred_label < num_label:
                    ...
                    sentence_results.append((pred_score, m1, m2, pred_label, v2_sub_label, v2_obj_label))
...
def main():
    ...
    parser.add_argument("--max_ent_length", default=10, type=int, help="一个句子中最多的实体个数")
    parser.add_argument("--shiyanfangshi", default=1, type=int,
                        help="第一种实验方式表示ent-marker只能看到tokens，而且re-marker只能看到对应的ent-marker，第二种实验方式表示ent-marker只能看到tokens，但是re-marker能够看到所有的ent-marker,第三种方式表示ent-marker能看到所有的tokens和re-marker,但是re-marker只能看到对应的ent-marker,第四种方式类似于3，但是re-marker能够看到所有的entmarker,第五种方式re-marker不能看到ent-marker,用于类比对照")
    parser.add_argument("--use_ner_loss",action='store_true', help="是否引入nerloss")
    ...
    config.max_ent_length = args.max_ent_length
