    ...
    def __init__(self, tokenizer, args=None, evaluate=False, do_test=False, max_pair_length=None):
        ...
        if args.data_dir.find('ace05') != -1:
            ...
            if args.no_sym:
                ...
                self.label_list = self.sym_labels + label_list
        elif args.data_dir.find('hwamei') != -1:
    ...
    def initialize(self):
        ...
        for l_idx, line in enumerate(f):
            ...
            for sentence_relation in relations:
                head_ids = sentence_relation['head']
                tail_ids = sentence_relation['tail']
                re_label = sentence_relation['type']
                head = std_ners[head_ids]
                tail = std_ners[tail_ids]
                sentence_relations.append((head['start'], head['end'], tail['start'], tail['end'], re_label))
                # self.tot_recall +=  1#数据集里面有重复的
            ...
            for x in sentence_relations:
                ...
                self.golden_labels.add(((l_idx, 0), (x[0], x[1]), (x[2], x[3]), x[4]))
                self.golden_labels_withner.add(((l_idx, 0), (x[0], x[1], std_entity_labels[(x[0], x[1])]),
        ...
        self.tot_recall = len(self.golden_labels)
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    results = {'f1': f1, 'f1_with_ner': f1_with_ner, 'ner_f1': ner_f1, 'precision':p, 'recall':r, 'p_with_ner':p_with_ner, 'r_with_ner':r_with_ner}
...
def main():
    ...
    parser.add_argument('--dont_use_ner_loss',action='store_true',help="不通过loss的方式引入ner label")

    parser.add_argument('--add_lstm_for_re', action='store_true')
    parser.add_argument('--add_binglixinxi', type=int, default=0, help='加入病历信息，0表示不加入，1表示加入')#注意这里不想ner中引入的方式中采用了loss的方法引入，而是直接在编码过程中引入
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        json.dump(results, open(output_eval_file, "w", encoding='utf-8'))
