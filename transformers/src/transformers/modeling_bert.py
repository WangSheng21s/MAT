...
from torch.nn import CrossEntropyLoss, MSELoss, BCEWithLogitsLoss, MultiLabelSoftMarginLoss
...
class BertForACEBothOneDropoutSub(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.add_lstm_for_re = config.add_lstm_for_re
        self.dont_use_ner_loss = config.dont_use_ner_loss
        self.hidden_size = config.hidden_size

        if self.add_lstm_for_re:
            num_layers = 1
            self.hidden_size = 300
            self.lstm_layer = nn.LSTM(input_size=config.hidden_size, hidden_size=self.hidden_size , num_layers=num_layers,
                                       bias=True, batch_first=True, dropout=0, bidirectional=True)#双向LSTM得到的hidden_size会乘以2
            self.dropout_1 = nn.Dropout(p = 0.3)
            #self.dropout_2 = nn.Dropout( p=0.3 )

            self.ner_classifier = nn.Linear(self.hidden_size * 4, self.num_ner_labels)

            self.re_classifier_m1 = nn.Linear(self.hidden_size * 4, self.num_labels)
            self.re_classifier_m2 = nn.Linear(self.hidden_size * 4, self.num_labels)
        else:

            self.ner_classifier = nn.Linear(self.hidden_size * 2, self.num_ner_labels)

            self.re_classifier_m1 = nn.Linear(self.hidden_size * 2, self.num_labels)
            self.re_classifier_m2 = nn.Linear(self.hidden_size * 2, self.num_labels)
    ...
    def forward(
        ...
        hidden_states = outputs[0]
        hidden_states = hidden_states[:,:-2,:]#丢掉最后两个marker因为是加过来添加额外病历信息的

        if self.add_lstm_for_re:
            #hidden_states.transpose(0, 1)#lstm默认是batch在第二个维度
            hidden_states = self.dropout_1(hidden_states)
            hidden_states, (h_n, c_n) = self.lstm_layer(hidden_states)
            #hidden_states.transpose(0, 1)
        hidden_states = self.dropout(hidden_states)
        seq_len = self.max_seq_length
        bsz, tot_seq_len = input_ids.shape
        ent_len = (tot_seq_len - seq_len - 2) // 2#注意这里还需要-2，原因和hidden_states = hidden_states[:,:-2,:]#丢掉最后两个marker因为是加过来添加额外病历信息的
        ...
        if labels is not None:
            ...
            re_loss = loss_fct_re(re_prediction_scores.view(-1, self.num_labels), labels.view(-1))
            ner_loss = loss_fct_ner(ner_prediction_scores.view(-1, self.num_ner_labels), ner_labels.view(-1))
            #print(re_loss.size(), ner_loss.size())

            loss = re_loss + ner_loss
            if self.dont_use_ner_loss:
                loss = re_loss
            outputs = (loss, re_loss, ner_loss) + outputs
...
class BertForHwaMeiAE(BertPreTrainedModel):#用于hwamei数据集做属性抽取
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_attr_labels = config.num_attr_labels#hwamei数据集对应的属性抽取的标签种类数量
        self.num_ner_labels = config.num_ner_labels
        ...
        self.ner_classifier = nn.Linear(self.hidden_size * 2, self.num_ner_labels)
        self.ae_classifier_m1 = nn.Linear(self.hidden_size * 2, self.num_attr_labels)
        self.ae_classifier_m2 = nn.Linear(self.hidden_size * 2, self.num_attr_labels)
        self.bingli_classifier = nn.Linear(self.hidden_size * 2, self.num_attr_labels)

        self.alpha = torch.tensor([config.alpha] + [1.0] * (self.num_attr_labels - 1), dtype=torch.float32)

        self.init_weights()
        self.add_binglixinxi = config.add_binglixinxi
        self.dont_use_ner_loss = config.dont_use_ner_loss
    ...
    def forward(
        ...
        seq_len = self.max_seq_length
        bsz, tot_seq_len = input_ids.shape
        ent_len = (tot_seq_len - seq_len - 2) // 2

        e1_hidden_states = hidden_states[:, seq_len:seq_len + ent_len]
        e2_hidden_states = hidden_states[:, seq_len + ent_len:-2]

        feature_vector = torch.cat([e1_hidden_states, e2_hidden_states], dim=2)

        bingli_hidden_state1 = hidden_states[:, -2]
        bingli_hidden_state2 = hidden_states[:, -1]
        bingli_feature = torch.cat([bingli_hidden_state1, bingli_hidden_state2], dim = 1)
        bingli_scores = self.bingli_classifier(bingli_feature)

        ner_prediction_scores = self.ner_classifier(feature_vector)

        m1_start_states = e1_hidden_states
        m1_end_states = e2_hidden_states
        for i in range(bsz):
            m1_start_states[i] = hidden_states[i, mention_pos[i, :, 0]]
            m1_end_states[i] = hidden_states[i, mention_pos[i, :, 1]]
        m1_states = torch.cat([m1_start_states, m1_end_states], dim=-1)

        m1_scores = self.ae_classifier_m1(m1_states)  # bsz, num_label
        m2_scores = self.ae_classifier_m2(feature_vector)  # bsz, ent_len, num_label
        ae_prediction_scores = m1_scores + m2_scores

        #if self.add_binglixinxi == 3:
            #ae_prediction_scores = ae_prediction_scores + bingli_scores.unsqueeze(1)

        outputs = (ae_prediction_scores, ner_prediction_scores) + outputs[
                                                                  2:]  # Add hidden states and attention if they are here

        if attr_labels is not None:
            #loss_fct_ae = CrossEntropyLoss(ignore_index=-1, weight=self.alpha.to(re_prediction_scores))
            loss_fct_ae = MultiLabelSoftMarginLoss(reduction='mean', weight=self.alpha.to(ae_prediction_scores))
            loss_fct_ner = CrossEntropyLoss(ignore_index=-1)
            ae_loss = loss_fct_ae(ae_prediction_scores.view(-1, self.num_attr_labels), attr_labels.view(-1, self.num_attr_labels))
            ner_loss = loss_fct_ner(ner_prediction_scores.view(-1, self.num_ner_labels), ner_labels.view(-1))
            #print(re_loss.size(), ner_loss.size())

            loss = ae_loss + ner_loss
            if self.dont_use_ner_loss:
                loss = ae_loss
            outputs = (loss, ae_loss, ner_loss) + outputs

        return outputs  # (masked_lm_loss), prediction_scores, (hidden_states), (attentions)



class BertFor_n_span_marker_Sub(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
    ...
    def forward(
        ...
        e1_hidden_states = hidden_states[:, seq_len:seq_len + ent_len]
        e2_hidden_states = hidden_states[:, seq_len + ent_len:]
        obj_feature_vector = torch.cat([e1_hidden_states, e2_hidden_states], dim=2)
        #print(sub_positions.size())
        #print(sub_positions)

        e3_hidden_states = e1_hidden_states
        e4_hidden_states = e2_hidden_states
        for i in range(len(hidden_states)):
            e3_hidden_states[i] = hidden_states[i, sub_positions[i, :, 0]]
            e4_hidden_states[i] = hidden_states[i, sub_positions[i, :, 1]]
        sub_feature_vector = torch.cat([e3_hidden_states, e4_hidden_states], dim=2)
        #print(sub_feature_vector.size(), obj_feature_vector.size())

        ner_prediction_scores = self.ner_classifier(obj_feature_vector)

        #m1_start_states = hidden_states[torch.arange(bsz), sub_positions[:, 0]]
        #m1_end_states = hidden_states[torch.arange(bsz), sub_positions[:, 1]]
        #m1_states = torch.cat([m1_start_states, m1_end_states], dim=-1)

        m1_scores = self.re_classifier_m1(sub_feature_vector)  # bsz, num_label
        m2_scores = self.re_classifier_m2(obj_feature_vector)  # bsz, ent_len, num_label
        re_prediction_scores = m1_scores + m2_scores
...
class BertForSpanMarkerNER(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bingli_classifier = nn.Linear(config.hidden_size * 2, self.num_labels)

        self.alpha = torch.tensor([config.alpha] + [1.0] * (self.num_labels - 1), dtype=torch.float32)
        self.onedropout = config.onedropout
        self.binglixinxi2 = config.binglixinxi2
    ...
    def forward(
        ...
        seq_len = self.max_seq_length
        bsz, tot_seq_len = input_ids.shape
        ent_len = (tot_seq_len - seq_len - 2) // 2#这里还要-2因为额外插入了两个marker用来存放电子病历信息

        e1_hidden_states = hidden_states[:, seq_len:seq_len + ent_len]
        e2_hidden_states = hidden_states[:, seq_len + ent_len:-2]
        department_embedding = hidden_states[:, -2]
        weizhi_embedding = hidden_states[:, -1]#batch*hidden_size
        bingli_embedding = torch.cat([department_embedding, weizhi_embedding], dim=1)
        bingli_scores = self.bingli_classifier(bingli_embedding)
        ...
        #print(e1_hidden_states.size(), e2_hidden_states.size(), m1_start_states.size(), m1_end_states.size())

        feature_vector = torch.cat([e1_hidden_states, e2_hidden_states, m1_start_states, m1_end_states], dim=2)
        if not self.onedropout:
            feature_vector = self.dropout(feature_vector)
        ner_prediction_scores = self.ner_classifier(feature_vector)
        #print(bingli_scores.size(), ner_prediction_scores.size())
        ner_scores =  ner_prediction_scores
        if self.binglixinxi2 == 3:
            ner_scores = bingli_scores.unsqueeze(1) + ner_prediction_scores

        outputs = (ner_scores,) + outputs[2:]  # Add hidden states and attention if they are here

        if labels is not None:
            loss_fct_ner = CrossEntropyLoss(ignore_index=-1, weight=self.alpha.to(ner_scores))
            ner_loss = loss_fct_ner(ner_scores.view(-1, self.num_labels), labels.view(-1))
            outputs = (ner_loss,) + outputs
